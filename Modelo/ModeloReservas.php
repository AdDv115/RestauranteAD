...
class Reservas {
    ...
    public function RegistrarReservas($NumeroPersonas, $FechaReserva, $HoraReserva, $Estado, $ID_User, $ID_M) {
        $sql = "INSERT INTO reservas(NumeroPersonas, FechaReserva, HoraReserva, Estado, ID_User, ID_M) 
                VALUES (:numeropersonas, :fechareserva, :horareserva, :estado, :id_user, :id_m)";
    ...
    public function actualizarReservas($id, $NumeroPersonas, $FechaReserva, $HoraReserva, $Estado, $ID_M) {
        $sql = "UPDATE reservas 
                SET NumeroPersonas = ?, FechaReserva = ?, HoraReserva = ?, Estado = ?, ID_M = ?  
                WHERE ID_RE = ?";
        $stmt = $this->db->prepare($sql);
        return $stmt->execute([$NumeroPersonas, $FechaReserva, $HoraReserva, $Estado, $ID_M, $id]);
    }
